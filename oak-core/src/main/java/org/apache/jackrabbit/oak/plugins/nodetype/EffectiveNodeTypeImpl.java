...
class EffectiveNodeTypeImpl implements EffectiveNodeType {
    ...
    public void checkSetProperty(PropertyState property) throws RepositoryException {
        ...
        if (definition.isMultiple()) {
            ...
            ValueFactory valueFactory = ntMgr.getValueFactory();
            Value v;
            if (valueFactory instanceof ValueFactoryImpl) {
                v = ((ValueFactoryImpl) valueFactory).createValue(property);
            }
            else {
                v = ValueFactoryImpl.createValue(property, ntMgr.getNamePathMapper());
            }
