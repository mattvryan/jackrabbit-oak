...
public interface CloudBackend extends SharedBackend {
    ...
    @Nullable URI createHttpDownloadURI(
            @NotNull DataIdentifier identifier,
            @NotNull DataRecordDownloadOptions options
    );

    void setHttpUploadURIExpirySeconds(int seconds);
    void setHttpUploadBinaryTransferAccelerationEnabled(boolean enabled);
    @Nullable DataRecordUpload initiateHttpUpload(
            long maxUploadSizeInBytes,
            int maxNumberOfURIs
    );
    @NotNull DataRecord completeHttpUpload(@NotNull String uploadToken)
